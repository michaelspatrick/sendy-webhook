...
class API {
  ...
  if ($_REQUEST['listID']) $listID = $_REQUEST['listID']; else $listID = "dCVaTOBI6omd5eQKT3BLLA";
  $config = [
    'sendyUrl' => 'https://email.dragonsociety.com', // Your Sendy installation URL (without trailing slash).
    'apiKey'   => 'AizKQZYnvgyzox72AAlQ', // Your API key. Available in Sendy Settings.
    'listId'   => $listID,
  ];
  $sendy = new \DSI\Sendy\API( $config );

  // --------------------------- ADD EMAIL TO SENDY LIST ---------------------------
  if (isset($_REQUEST['Email'])) $email = $_REQUEST['Email'];
    elseif (isset($_REQUEST['Email_Address'])) $email = $_REQUEST['Email_Address'];
      elseif (isset($json['email'])) $email = $json['email'];
      
  if (isset($email)) {
    $responseArray = $sendy->subscribe([
      'email'     => $email, // This is the only field required by sendy.
      'referrer'  => $_SERVER['HTTP_REFERER'], // URL where the user signed up from (optional).
    ]);
  }
?>
